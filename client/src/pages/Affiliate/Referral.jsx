...
function getInitials(item) {
  const displayName = String(item?.full_name || item?.username || item?.email || "User").trim();
...
export default function Referral() {
  ...
  const simpleReferralUsers = useMemo(() => referredUsers.slice(0, 8), [referredUsers]);
  const simpleReferralDisplayUsers =
    referredUsers.length > 3 ? [...simpleReferralUsers, ...simpleReferralUsers] : simpleReferralUsers;
  ...
  const loadAffiliate = useCallback(async () => {
    ...
    try {
      ...
      setError(err?.response?.data?.message || "Unable to load referral page.");
    } finally {
      setLoading(false);
    }
  }, []);
      ...
      <main className={styles.main}>
        <div className={styles.topBar}>
          ...
          <button
            ...
            disabled={loading}
            aria-label="Refresh referral page"
          >
            <FiRefreshCw />
          </button>
        </div>
        ...
        <section className={styles.hero}>
          <div>
            <p className={styles.kicker}>Referral Tools</p>
            <h1>Share links. Grow your network.</h1>
            <p>
              Share your signup referral link to bring in users, then generate heist links when you
              want ticket activity to count from your referred users.
            </p>
          </div>
          ...
          <div>
            ...
            {rewardSystemEnabled ? (
              <p className={styles.sectionText}>
                ...
                Claim each user once they hit the target.
              </p>
            ) : null}
          </div>
        </section>
            ...
            rewardSystemEnabled ? (
              <div className={styles.referredScroller}>
                {referredUsers.map((item) => {
                  ...
                  const claimLocked = !canClaim || isClaimed;
              ...
              <div className={styles.simpleReferralViewport}>
                <div
                  className={`${styles.simpleReferralTrack} ${
                    referredUsers.length > 3 ? styles.simpleReferralTrackAnimated : ""
                  }`}
                >
                  {simpleReferralDisplayUsers.map((item, index) => (
                    <article
                      className={`${styles.referredCard} ${styles.simpleReferredCard}`}
                      key={`${item.id}-${index}`}
                    >
                      <div className={styles.simpleReferralContent}>
                        <div className={styles.simpleAvatar} aria-hidden="true">
                          {getInitials(item)}
                        </div>
                      ...
                      <div className={styles.simpleReferralAction}>
                        <span>{item.username ? `@${item.username}` : `User #${item.user_id}`}</span>
